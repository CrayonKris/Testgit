	...
	$(function(){
		$(".del").click(function(){
			if(confirm("您确定删除吗？")){
				//获取订单id
				var id = $(this).attr("id");
				location.href="${app}/OrderDeleteServlet?id="+id;
			}
		});
	});
	</script>
	...
	<dd>
		订单编号：${info.order.id }<br />
		 ...
		 <c:if test="${info.order.paystate==0}">
			...
			<a class="del" id="${info.order.id }" href="javascript:void(0)"><img src="img/orderList/sc.jpg" width="69" height="19"></a> 
	 		<a href="${app}/pay.jsp?id=${info.order.id}&money=${info.order.money}"> <img src="img/orderList/zx.jpg" width="69" height="19"></a><br /> 
		 </c:if>
